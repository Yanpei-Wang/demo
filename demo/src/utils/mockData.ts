import { UserData, UserRole, MentorshipMeeting, MentorshipRound, PartnerDetails, EmployeeLevel, LeaveBalance, LeaveRequest, QuarterlyHoliday, Training, TrainingCategory, TrainingStatus } from '../types/dashboard';

const ALL_TRAINING_CATEGORIES: TrainingCategory[] = [
...
const TRAINING_STATUSES: TrainingStatus[] = ['to_do', 'in_progress', 'done'];

function generateTrainings(role: UserRole): Training[] {
  const relevant: TrainingCategory[] = ['corporate_culture_course'];
  if (role === 'circlecat_intern') relevant.push('residency_program_onboarding');
  if (role === 'circlecat_employee' || role === 'admin') {
    relevant.push('mentorship_mentee_onboarding');
    if (Math.random() > 0.5) relevant.push('mentorship_mentor_onboarding');
  }
  if (role === 'circlecat_volunteer' || role === 'googler') {
    relevant.push('mentorship_mentor_onboarding');
  }
  if (role === 'external_mentee') relevant.push('mentorship_mentee_onboarding');
  return relevant.map((category) => ({
    category,
    status: TRAINING_STATUSES[Math.floor(Math.random() * TRAINING_STATUSES.length)],
  }));
}
...
export const generateMockUserData = (role: UserRole, isCurrentUser: boolean = false, index?: number): UserData => {
  ...
  return {
    ...
    trainings: generateTrainings(role),
  };
};
...
export const generateMockDataset = (): UserData[] => {
  ...
  data.push({
    ...
    trainings: [
      { category: 'corporate_culture_course', status: 'done' },
      { category: 'mentorship_mentee_onboarding', status: 'in_progress' },
    ],
  } as any);
